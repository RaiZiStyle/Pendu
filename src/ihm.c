...
void curses_init(void) {
    ...
    // Permet la capture des touches spéciales telles que Backspace, Delete, F1,
    // F2, ... et les touches directionnelles
    keypad(stdscr, TRUE);
...
void init_fenetre() {
    ...
    if (sig_ok == SIG_ERR) {
        clear();
        mvprintw(fenetre->fenetre_h0, fenetre->fenetre_l0,
                 "Le gestionnaire de signal ne s'est pas initialisé.");
        mvprintw(
            fenetre->fenetre_h3, fenetre->fenetre_l0,
            "Le redimensionnement de la fenêtre ne sera pas pris en compte.");
        mvprintw(fenetre->fenetre_h5, fenetre->fenetre_l0,
...
void curses_fenetre_constantes(int signal) {
    ...
    fenetre->menu_demander_mot_largeur_nombre =
        fenetre->menu_demander_mot_largeur + 4;
    fenetre->menu_demander_mot_largeur_lettres =
        fenetre->menu_demander_mot_largeur + 33;
    fenetre->menu_demander_mot_hauteur = (short)h / 4;
    fenetre->menu_demander_mot_hauteur_invite =
        fenetre->menu_demander_mot_hauteur + 1;
    fenetre->menu_demander_mot_hauteur_lettres =
        fenetre->menu_demander_mot_hauteur + 3;
    fenetre->menu_demander_mot_hauteur_message =
        fenetre->menu_demander_mot_hauteur + 5;
...
short ecran_accueil(FENETRE_PENDU *fenetre) {
    ...
    attron(COLOR_PAIR(2));
    mvprintw(fenetre->menu_h2, fenetre->menu_l2,
             "Voir les meilleurs scores (F2)");
...
short ecran_niveau(FENETRE_PENDU *fenetre) {
    ...
    attron(A_BOLD);
    mvprintw(fenetre->menu_niveau_hauteur, fenetre->menu_niveau_largeur,
             "Choisissez le niveau:");
    attroff(A_BOLD);

    mvprintw(fenetre->menu_niveau_hauteur_1, fenetre->menu_niveau_largeur,
             "[ ] Rapide     3 mots");
    mvprintw(fenetre->menu_niveau_hauteur_2, fenetre->menu_niveau_largeur,
             "[ ] Medium     5 mots");
    mvprintw(fenetre->menu_niveau_hauteur_3, fenetre->menu_niveau_largeur,
             "[ ] Endurant  10 mots");

    attron(A_BOLD | COLOR_PAIR(3));
    mvprintw(fenetre->menu_niveau_hauteur_1,
             fenetre->menu_niveau_largeur_chiffre, "1");
    mvprintw(fenetre->menu_niveau_hauteur_2,
             fenetre->menu_niveau_largeur_chiffre, "2");
    mvprintw(fenetre->menu_niveau_hauteur_3,
             fenetre->menu_niveau_largeur_chiffre, "3");
    attroff(A_BOLD | COLOR_PAIR(3));
...
char *ecran_demander_mot(FENETRE_PENDU *fenetre) {
    ...
    attron(A_BOLD);
    mvprintw(fenetre->menu_demander_mot_hauteur,
             fenetre->menu_demander_mot_largeur,
             "Veuillez saisir le mot à trouver");
    attroff(A_BOLD);

    mvprintw(fenetre->menu_demander_mot_hauteur_invite,
             fenetre->menu_demander_mot_largeur, "> ");
    ...
    attron(A_BOLD);
    mvprintw(fenetre->menu_demander_mot_hauteur_lettres,
             fenetre->menu_demander_mot_largeur,
             "Les    lettres à trouver sont :", nb_a_trouver, a_trouver);
    attron(COLOR_PAIR(3));
    mvprintw(fenetre->menu_demander_mot_hauteur_lettres,
             fenetre->menu_demander_mot_largeur_nombre, "%02d", nb_a_trouver);
    mvprintw(fenetre->menu_demander_mot_hauteur_lettres,
             fenetre->menu_demander_mot_largeur_lettres, "%s", a_trouver);
    attroff(A_BOLD | COLOR_PAIR(3));

    mvprintw(fenetre->menu_demander_mot_hauteur_message,
             fenetre->menu_demander_mot_largeur,
...
void ecran_afficher_chaine_masquee(FENETRE_PENDU *fenetre, const char *chaine,
                                   const char *trouves) {
    attron(A_BOLD | COLOR_PAIR(1));
    ...
    for (i = 0; i < taille; i++) {
        if (strchr(trouves, chaine[i]) == NULL) {
            mvprintw(fenetre->chaine_masquee_hauteur,
                     fenetre->chaine_masquee_largeur + i, "_");
        } else {
            mvprintw(fenetre->chaine_masquee_hauteur,
                     fenetre->chaine_masquee_largeur + i, "%c", chaine[i]);
        }
    }
...
void ecran_afficher_chaine_reponse(FENETRE_PENDU *fenetre, const char *chaine,
                                   const char *trouves) {
    int taille = strlen(chaine), i;
    for (i = 0; i < taille; i++) {
        if (strchr(trouves, chaine[i]) == NULL) {
...
void ecran_afficher_pendu(FENETRE_PENDU *fenetre, short echecs) {
    switch (echecs) {
        ...
        case 1: {
            mvprintw(fenetre->fenetre_hauteur_4, fenetre->pendu_l0,
                     "==================");
            mvprintw(fenetre->fenetre_hauteur_3, fenetre->pendu_l0,
                     "XX              XX");
            mvprintw(fenetre->fenetre_hauteur_2, fenetre->pendu_l0,
                     "XX              XX");
        }
    }
    ...
    if (echecs == 12) {
        ...
        mvprintw(fenetre->fenetre_hauteur_5, fenetre->pendu_l10,
                 " d b ");  // 12
    }
}
...
int ecran_jeu(FENETRE_PENDU *fenetre, char *mot) {
    ...
    while (echecs < 12) {
        ...
        while (essai < 97 || essai > 122) {
            ...
            if (caractere_dans_chaine(essayes, essai)) {
                ...
                mvprintw(fenetre->menu_demander_mot_hauteur_lettres,
                         fenetre->menu_demander_mot_largeur,
                         "Ce caractère a déjà été essayé");
            }
        }
        ...
        if (caractere_dans_chaine(mot, essai)) {
            ...
            mvprintw(fenetre->menu_demander_mot_hauteur_lettres,
                     fenetre->menu_demander_mot_largeur,
                     "%c n'est pas dans le mot mystère !", essai);
        }
    ...
    mvprintw(fenetre->menu_demander_mot_hauteur,
             fenetre->menu_demander_mot_largeur, "Vous avez mis %.2f secondes",
    ...
    if (echecs == 12) {
        ...
        mvprintw(fenetre->menu_demander_mot_hauteur_invite,
                 fenetre->menu_demander_mot_largeur,
                 "Vous n'avez pas trouvé le mot: Malus de 5 minutes !");
        // Ajout d'un malus
        temps_fin += 300;
    } else {
        temps_fin += echecs;
        mvprintw(fenetre->menu_demander_mot_hauteur_invite,
                 fenetre->menu_demander_mot_largeur, "Le mot est trouvé !");
    }
...
char *ecran_resultat(FENETRE_PENDU *fenetre, int score) {
    ...
    attron(A_BOLD);
    mvprintw(fenetre->menu_demander_mot_hauteur,
             fenetre->menu_demander_mot_largeur, "Votre score total est %d",
             score);
    attroff(A_BOLD);
...
void ecran_scores(FENETRE_PENDU *fenetre, TABLEAU_SCORE *tableau1,
                  TABLEAU_SCORE *tableau2, TABLEAU_SCORE *tableau3) {
    char choix = '1';
    int i;
    ...
    while (1) {
        ...
        if (score_courant == NULL) {
            mvprintw(1, fenetre->tableau_score_largeur,
                     "Pas encore de scores enregistrés.");
        } else {
            mvprintw(fenetre->fenetre_h0, fenetre->tableau_score_largeur,
                     "+---------------------------------+-----+");
            mvprintw(fenetre->fenetre_h1, fenetre->tableau_score_largeur,
                     "| Joueur                          |Score|");
        ...
        mvprintw(fenetre->fenetre_hauteur_5, fenetre->fenetre_l0,
                 "Appuyez sur [1], [2] ou [3] pour changer de niveau.");
        mvprintw(fenetre->fenetre_hauteur_4, fenetre->fenetre_l0,
...
LISTE_MOT *curses_manage_arguments(FENETRE_PENDU *fenetre, int argc,
                                   char *argv[]) {
    ...
    if (argc == 2) {
        if (strcmp(argv[1], "-h") == 0 || strcmp(argv[1], "--help") == 0) {
            ...
            printf("usage: %s [filename]\n\n", argv[0]);
            printf(
                "Pour jouer seul face à l'ordinateur:    %s "
                "fichier_dictionnaire.txt\n",
                argv[0]);
        ...
        if (errno != 0) {
            ...
            mvprintw(fenetre->fenetre_h0, fenetre->fenetre_l0,
                     "Une erreur s'est produite dans le chargement du fichier "
                     "%s: %s",
                     argv[1], erreur_label);
            mvprintw(fenetre->fenetre_h3, fenetre->fenetre_l0,
