...
char *saisir_chaine(void) {
    // Saisie aléatoire (on considère que la longueux maximale d'un mot est de
    // 32 caractères).
    char *saisie, *chaine;
...
void rand_init(void) { srand((unsigned)time(NULL)); }
void melanger_mots(LISTE_MOT *liste_mots) {
    ...
    int nombre = liste_mots->nombre,
        nombre_boucles = 2 * nombre + rand() % nombre, i, index1, index2;
 ...
 *                  *
 *******************/
void ajouter_score(TABLEAU_SCORE *tableau, int nouveau_score_valeur,
                   char *nom_joueur) {
    // !!!
    ...
    */
    ...
    {
        // printf("Nouveau score: %d %p %p\n", nouveau_score->valeur,
        // nouveau_score->inferieur, nouveau_score->superieur);
    ...
    if (nouveau_score_valeur >= score_avant->valeur) {
        ...
        // Sinon, le tableau est plein et l'on n'a pas fait mieux (on retourne
        // sans rien faire)
    ...
    while (nouveau_score_valeur < score_avant->valeur) {
        ...
        // nouveau_score_valeur, score_avant->valeur, score_avant->inferieur,
    ...
    // A ce stade, nous avons trouvé le premier score égal ou meilleur que le
    // nouveau score On va donc reprendre le score juste en dessous.
    score_apres = score_avant->inferieur;

    // printf("TROUVE AVANT : %d (%p > %p > %p)\n", score_avant->valeur,
    // score_avant->inferieur, score_avant, score_avant->superieur);
    // printf("TROUVE APRES : %d (%p > %p > %p)\n", score_apres->valeur,
    // score_apres->inferieur, score_apres, score_apres->superieur);
...
void afficher_tableau_score(TABLEAU_SCORE *tableau) {
    ...
    do {
        ...
        // score_courant->superieur, score_courant, score_courant->inferieur);
...
void db_create(const char *db_filename) {
    ...
    if (rc) {
        fprintf(stderr, "Ouverture de la base de données %s impossible: %s\n",
...
void db_set(const char *db_filename, int niveau, TABLEAU_SCORE *tableau) {
    ...
    int rc;
    SCORE *score_courant = tableau->premier;
    static char *base_sql =
        "INSERT INTO SCORE (NIVEAU, NOM, SCORE) VALUES (?, ?, ?);";
    ...
    if (rc != SQLITE_OK) {
        fprintf(stderr,
                "Problème lors de la préparation de la requête SQL %s pour la "
                "base de donnéee %s:\n",
                delete_sql, db_filename);
        exit(1);
    }
    ...
    if (rc != SQLITE_OK) {
        fprintf(stderr,
                "Problème lors du bind de %d à la requête %s pour la base de "
                "donnéee %s:\n",
                niveau, delete_sql, db_filename);
        exit(1);
    }
    ...
    while (1) {
        ...
        if (rc == SQLITE_DONE) {
            ...
            fprintf(stderr,
                    "Problème lors de la suppression des données de requête %s "
                    "pour la base de donnéee %s:\n",
                    delete_sql, db_filename);
            exit(1);
        }
    }
    ...
    do {
        ...
        if (rc != SQLITE_OK) {
            fprintf(stderr,
                    "Problème lors du bind du niveau %d à la requête %s pour "
                    "la base de donnéee %s:\n",
                    niveau, base_sql, db_filename);
            exit(1);
        }

        // Bind du paramètre 'nom'
        rc =
            sqlite3_bind_text(stmt, 2, score_courant->nom_joueur,
                              strlen(score_courant->nom_joueur), SQLITE_STATIC);
        if (rc != SQLITE_OK) {
            fprintf(stderr,
                    "Problème lors du bind du nom du joueur %s à la requête %s "
                    "pour la base de donnéee %s:\n",
                    score_courant->nom_joueur, base_sql, db_filename);
            exit(1);
        }
        ...
        if (rc != SQLITE_OK) {
            fprintf(stderr,
                    "Problème lors du bind du score %d à la requête %s pour la "
                    "base de donnéee %s:\n",
                    score_courant->valeur, base_sql, db_filename);
            exit(1);
        }
        ...
        while (1) {
            ...
            if (rc == SQLITE_DONE) {
                ...
                fprintf(stderr,
                        "Problème lors de l'ajout des données de requête %s "
                        "pour la base de donnéee %s:\n",
                        base_sql, db_filename);
                exit(1);
            }
...
TABLEAU_SCORE *db_get(const char *db_filename, int niveau) {
    ...
    int rc;
    static char *base_sql =
        "SELECT NOM, SCORE FROM SCORE WHERE NIVEAU = ? ORDER BY SCORE ASC "
        "LIMIT 10;";
    sqlite3_stmt *stmt;
    ...
    while (1) {
        ...
        if (rc == SQLITE_DONE) {
            ...
            fprintf(stderr,
                    "Problème lors de la lecture des données de requête %s "
                    "pour la base de donnéee %s:\n",
                    base_sql, db_filename);
            exit(1);
        }
...
short menu() {
    ...
    do {
        ...
        switch (choix) {
            ...
            default: {
                printf(
                    "Nous n'avons pas compris la réponse, merci de la saisir à "
                    "nouveau.\n");
                break;
            }
        }
...
int jeu(const char *mot) {
    ...
    while (echecs < 12) {
        ...
        if (caractere_dans_chaine(mot, essai)) {
            ...
            printf(
                "Ce caractère n'est pas contenu dans le mot mystère ! Erreur "
                "n°%d\n",
                echecs);
        }
    }
...
void scores(TABLEAU_SCORE *tableau_scores_1, TABLEAU_SCORE *tableau_scores_2,
            TABLEAU_SCORE *tableau_scores_3) {
